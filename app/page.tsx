    ...
    <>
      <main className="flex-1 px-6 pt-12 pb-28">
        <header className="mb-10 flex items-start justify-between gap-4">
          ...
          <Link
            href="/account"
            className="text-sm text-white/60 shrink-0 mt-1"
          >
            Account
          </Link>
        </header>
        ...
        <Link
          href="/surgeons"
          className="block w-full rounded-xl bg-accent text-accent-dark font-semibold text-lg py-5 text-center mb-10"
        >
          Pick your surgeon
        </Link>
            ...
            <ul className="space-y-2">
              {upcoming.map((s) => (
                <li key={s.id}>
                  <Link
                    href={`/surgeons/${s.id}`}
                    className="block rounded-md border border-border bg-surface-card p-4"
                  >
